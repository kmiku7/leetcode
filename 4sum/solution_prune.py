...
class Solution:
    ...
    def fourSum(self, num, target):
        ...
        for idx_first in xrange(num_len):
            #if num[idx_first] > target:
            #    break
            for idx_second in xrange(idx_first+1, num_len):
                first_two_sum = num[idx_first] + num[idx_second]
                #if first_two_sum > target:
                #    break
                twoSum(num, idx_second+1, target - first_two_sum, [idx_first, idx_second], results)
        return results

def twoSum(num, begin, target, prefix, results):
    if prefix is None:
        prefix = []
    num_len = len(num)
    first_idx = begin
