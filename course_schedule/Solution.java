...
/**
 * 判断图里有没有环。
 * 1. 经过一条边并“返回”时，这条边的结果是可以缓存的。
 * 2. 可以初步检索是否存在入度为0的点，若不存在则一定false。
 * 3. 遍历起点不能仅仅限制在入度为0的点。
 * @author km
 *
 */
...
public class Solution {
	final private int DonotPre = 0;
	final private int Pre = 1;
	final private int Checked = 2;
	final private int Checking = 4;
	
    public boolean canFinish(int numCourses, int[][] prerequisites) {
    	int courseCount = 0;
    	for(int[] pre : prerequisites) {
    		assert(pre.length == 2);
    		courseCount = Math.max(Math.max(pre[0], pre[1]), courseCount);
    	}
    	courseCount += 1;
    	int[][] matrix = new int[courseCount][courseCount];
    	int[] preCount = new int[courseCount];
    	int notFinalCount = 0;
    	for(int[] pre: prerequisites) {
...
//        	}
    ...
    private boolean doCheck(int[][] matrix, int course) {
    	for(int i=0; i<matrix.length; i++) {
    		if(i==course || matrix[course][i] == DonotPre || (matrix[course][i] & Checked) != 0) continue;
    		if((matrix[course][i] & Checking) != 0) return false;
    		matrix[course][i] = Checking | Pre;
    		if(!doCheck(matrix, i)) return false;
    		matrix[course][i] = Checked | Pre;
    	}
    	return true;
    }
...
//    	int[][] matrix = new int[][]{{1,0},{0,1}};
    	...
    	// false
//    	int[][] matrix = new int[][]{{0,1},{1,2},{2,3},{3,4},{4,0}};
    	// false
//    	int[][] matrix = new int[][]{{0,2}, {1,2}, {2,0}};
    	// false
//    	int[][] matrix = new int[][]{
